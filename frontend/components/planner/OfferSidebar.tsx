...
export function OfferSidebar({ slots }: OfferSidebarProps) {
  ...
  return (
    <aside className="offer-sidebar" aria-label="Dostępne oferty">
      {offers.map((offer) => {
        const colors = getCompanyColorPair(offer.colorKey);
        return (
          <article
            key={offer.offerId}
